...
const ForecastCard: React.FC<{ forecast: AIForecast }> = ({ forecast }) => {
  ...
  return (
    <div className="bg-primary/5 p-4 rounded-lg">
      ...
      <p className="text-primary text-2xl font-bold my-1">
        {formatCurrency(forecast.predictedSpend)}
      </p>
      <p className="text-sm text-muted">
        Dự kiến từ {formatCurrency(forecast.confidenceInterval[0])} đến{' '}
        {formatCurrency(forecast.confidenceInterval[1])}
      </p>
    </div>
  );
};
...
const SuggestionCard: React.FC<{ suggestion: AISuggestion }> = ({ suggestion }) => {
  ...
  return (
    <div
      className={`border-l-4 ${priorityClasses[suggestion.priority]} bg-primary/5 p-4 rounded-r-lg`}
    >
...
const Premium: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const data = await apiService.getAIAnalysis(); // 👉 REAL DATABASE
        setAiData(data);
        setError(null);
      } catch (err) {
        console.error(err);
    ...
    <div>
      ...
      {isLoading && (
        <div className="text-center p-8">
          ...
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mt-4" />
        </div>
      )}

      {error && (
        <Card className="bg-danger/10 border border-danger text-danger">
          {error}
        </Card>
      )}
