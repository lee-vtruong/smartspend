...
import { auth } from "../firebaseClient"; // Đảm bảo đường dẫn đúng
...
const PasswordInput: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label: string; icon: React.ReactNode }> = ({ label, icon, ...props }) => {
        ...
        <div>
            ...
            <div className="relative mt-1">
                ...
                <input
                    ...
                    className="w-full pl-10 pr-12 py-2 bg-background border border-card-border rounded-lg focus:outline-none focus:ring-1 focus:border-primary"
                />
                <button
                    type="button"
...
const LoginView: React.FC<{ onSwitchToSignup: () => void; onSwitchToForgot: () => void; }> = ({ onSwitchToSignup, onSwitchToForgot }) => {
    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            ...
            if (response && response.requirePasswordSetup) {
    ...
    return (
        <form onSubmit={handleLogin} className="space-y-6">
            ...
            <PasswordInput label="Password" value={password} onChange={(e) => setPassword(e.target.value)} required icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>} />
...
const SignupView: React.FC<{ onSwitchView: () => void; onSignupSuccess: () => void; }> = ({ onSwitchView, onSignupSuccess }) => {
    // SỬA: Lấy thêm loginWithGoogle từ context
    const { signup, loginWithGoogle, t } = useAppContext(); 
    ...
    // SỬA: THÊM HÀM XỬ LÝ GOOGLE TRONG SIGNUP
    const handleGoogleLogin = async () => {
        setError('');
        setIsLoading(true);
        try {
         ...
         <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                <SocialButton
                    provider="Google"
                    icon={<svg className="w-5 h-5" viewBox="0 0 48 48"><path fill="#FFC107" d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8c-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4C12.955 4 4 12.955 4 24s8.955 20 20 20s20-8.955 20-20c0-1.341-.138-2.65-.389-3.917z"></path><path fill="#FF3D00" d="M6.306 14.691l6.571 4.819C14.655 15.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4C16.318 4 9.656 8.337 6.306 14.691z"></path><path fill="#4CAF50" d="M24 44c5.166 0 9.86-1.977 13.409-5.192l-6.19-5.238C29.211 35.091 26.715 36 24 36c-5.222 0-9.618-3.356-11.283-7.946l-6.522 5.025C9.505 39.556 16.227 44 24 44z"></path><path fill="#1976D2" d="M43.611 20.083H42V20H24v8h11.303c-.792 2.237-2.231 4.166-4.087 5.571l6.19 5.238C42.012 36.49 44 30.686 44 24c0-1.341-.138-2.65-.389-3.917z"></path></svg>}
                    // *** ĐÃ SỬA LẠI: Giờ đã có hàm handleGoogleLogin ***
                    onClick={handleGoogleLogin}
                />
                <SocialButton
                    provider="Facebook"
...
// Tìm component ForgotPasswordView và sửa lại logic

const ForgotPasswordView: React.FC<{ onSwitchView: () => void }> = ({ onSwitchView }) => {
    // Chúng ta không dùng handleResetPassword cũ nữa, mà gọi trực tiếp apiService hoặc cập nhật context
    // Ở đây mình giả sử bạn sửa trực tiếp trong View cho nhanh, hoặc update context sau.
    
    // Import hàm gửi mail từ firebase trực tiếp hoặc qua context
    const { t } = useAppContext();
    const [step, setStep] = useState<'enterEmail' | 'success'>('enterEmail'); // <--- BỎ BƯỚC OTP
    const [email, setEmail] = useState('');
    const [error, setError] = useState('');
    const [isLoading, setIsLoading] = useState(false);
    ...
    const handleEmailSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Gọi hàm gửi mail của Firebase (bạn có thể import trực tiếp ở đây cũng được)
            const { sendPasswordResetEmail } = await import("firebase/auth");
            const { auth } = await import("../firebaseClient");
            
            await sendPasswordResetEmail(auth, email);
            
            // Nếu thành công thì chuyển sang màn hình thông báo
            setStep('success');
        } catch (err: any) {
            console.error(err);
            ...
            case 'enterEmail':
                return (
                    <form onSubmit={handleEmailSubmit} className="space-y-6">
                        <h2 className="text-2xl font-bold text-center text-text">Quên mật khẩu?</h2>
                        {error && <p className="text-sm text-center text-danger bg-danger/10 p-2 rounded-lg">{error}</p>}
                        <p className="text-sm text-center text-muted">Nhập email của bạn, chúng tôi sẽ gửi liên kết để đặt lại mật khẩu.</p>
                        ...
                        <AuthInput 
                            ...
                            required 
                            icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" /></svg>} 
                        />
                        
                        <button type="submit" disabled={isLoading} className="w-full py-3 bg-primary text-primary-content rounded-lg font-semibold hover:bg-primary-focus flex justify-center items-center">
                             {isLoading ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-primary-content"></div> : "Gửi liên kết khôi phục"}
                        </button>
                        
                        <div className="text-center text-sm"><button type="button" onClick={onSwitchView} className="font-semibold text-primary hover:underline">Quay lại đăng nhập</button></div>
                    </form>
                );
            ...
            case 'success':
                 return (
                    <div className="text-center space-y-4 animate-fade-in-up">
                        ...
                        <h2 className="text-2xl font-bold text-text">Đã gửi email!</h2>
                        <p className="text-muted">
                            Vui lòng kiểm tra hộp thư đến (và cả mục spam) của <b>{email}</b>. Bấm vào liên kết trong mail để đặt lại mật khẩu mới.
                        </p>
                        <button onClick={onSwitchView} className="w-full py-3 bg-primary text-primary-content rounded-lg font-semibold hover:bg-primary-focus transition-opacity">
                            Quay lại đăng nhập
                        </button>
                    </div>
                 );
